...
const filePath = 'src/pages/บทความ/คำนวณภาษีพนักงานเงินเดือน-40000-บาท/index.astro';
assert.ok(fs.existsSync(filePath), 'income-tax example article page not found');
const source = fs.readFileSync(filePath, 'utf8');

const clusterId = 'income-tax-example-intent-cluster';
const requiredLink1 = '/คำนวณภาษีเงินได้บุคคลธรรมดา/';
const requiredLink2 = '/บทความ/ภาษีเงินได้-2569-คำนวณ-วิธีลดหย่อน/';

test('T044: income-tax example intent cluster exists', () => {
  assert.match(source, new RegExp(`<section id="${clusterId}"`, 'u'));
});

test('T044: cluster appears above first ArticleCalculatorCTA', () => {
...
test('T044: cluster contains both required links', () => {
...
test('T044: cluster id is unique', () => {
...
test('T044: faqData wiring remains intact', () => {
