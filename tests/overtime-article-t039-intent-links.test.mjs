...
const clusterId = 'overtime-secondary-intent-cluster';
const requiredLink1 = '/คำนวณค่าโอที/';
const requiredLink2 = '/บทความ/คำนวณค่าโอที-2569-กฎหมายแรงงาน/';

test('T039: overtime secondary intent cluster exists', () => {
  assert.match(source, new RegExp(`<section id="${clusterId}"`, 'u'));
});

test('T039: cluster appears above first ArticleCalculatorCTA', () => {
...
test('T039: cluster contains both required links', () => {
...
test('T039: cluster id is unique', () => {
...
test('T039: faqData wiring remains intact', () => {
