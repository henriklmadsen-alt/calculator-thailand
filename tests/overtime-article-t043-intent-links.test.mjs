...
const filePath = 'src/pages/บทความ/เงินเดือน-20000-โอที-10-ชั่วโมง/index.astro';
assert.ok(fs.existsSync(filePath), 'overtime example article page not found');
const source = fs.readFileSync(filePath, 'utf8');

const clusterId = 'overtime-example-intent-cluster';
const requiredLink1 = '/คำนวณค่าโอที/';
const requiredLink2 = '/บทความ/คำนวณค่าโอที-2569-กฎหมายแรงงาน/';

test('T043: overtime example intent cluster exists', () => {
  assert.match(source, new RegExp(`<section id="${clusterId}"`, 'u'));
});

test('T043: cluster appears above first ArticleCalculatorCTA', () => {
...
test('T043: cluster contains both required links', () => {
...
test('T043: cluster id is unique', () => {
...
test('T043: faqData wiring remains intact', () => {
