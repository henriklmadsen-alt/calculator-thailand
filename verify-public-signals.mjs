...
console.log(`OG Tags:          ${signals.og}/${count} (${(signals.og/count*100).toFixed(0)}%) ${signals.og >= count * 0.95 ? '✓' : '⚠'}`);
console.log(`Twitter Card:     ${signals.twitter}/${count} (${(signals.twitter/count*100).toFixed(0)}%) ${signals.twitter >= count * 0.95 ? '✓' : '⚠'}`);
console.log(`Schema.org:       ${signals.schema}/${count} (${(signals.schema/count*100).toFixed(0)}%) ${signals.schema >= count * 0.95 ? '✓' : '⚠'}`);
console.log(`GA4 Tracking:     ${signals.ga4}/${count} (${(signals.ga4/count*100).toFixed(0)}%) ${signals.ga4 >= count * 0.95 ? '✓' : '⚠'}`);
console.log(`Mobile Viewport:  ${signals.viewport}/${count} (${(signals.viewport/count*100).toFixed(0)}%) ${signals.viewport >= count * 0.95 ? '✓' : '⚠'}`);
console.log(`Sentry:           ${signals.sentry}/${count} (${(signals.sentry/count*100).toFixed(0)}%) ${signals.sentry >= count * 0.95 ? '✓' : '⚠'}`);
console.log(`PWA Manifest:     ${signals.pwa}/${count} (${(signals.pwa/count*100).toFixed(0)}%) ${signals.pwa >= count * 0.95 ? '✓' : '⚠'}`);
